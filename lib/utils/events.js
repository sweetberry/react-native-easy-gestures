...
export const getAngle = (event, styles, diffAngle, scalable) => {
  ...
  const currentAngle = parseFloat(transform.map((style) => style.rotate).reduce((a, b) => b || a, 0), 0);
  const resultAngle = (scalable.separate) ? currentAngle : currentAngle - diffAngle;
  return `${resultAngle}deg`;
};
