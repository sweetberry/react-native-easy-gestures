...
// Utils
import { angle, distance, distance2d } from './utils/math.js';
import {
  getAngle,
  getScale,
  getScaleX,
...
export default class Gestures extends Component {
  ...
  onRotate = (event) => {
    const { onRotateStart, onRotateChange, rotatable, scalable } = this.props;
    ...
    if (rotatable) {
      ...
      const tempNewAngle = getAngle(event, styles, diffAngle, scalable);
      this.pinchStyles.transform.push({
        rotate: `${tempNewAngle}deg`,
      });

      this.prevAngle = tempNewAngle;
  ...
  onScale = (event) => {
    ...
    if (isObject || scalable) {
      ...
      if (!scalable.separate) {
        ...
        const currentDistance2d = distance2d(getTouches(event));
        const initialDistance2d = distance2d(initialTouches);
        const increasedDistance2d = [currentDistance2d[0] - initialDistance2d[0], currentDistance2d[1] - initialDistance2d[1]];
        const diffDistance2d = [this.prevDistance2d[0] - increasedDistance2d[0], this.prevDistance2d[1] - increasedDistance2d[1]];

        const scaleX = Math.min(Math.max(getScaleX(event, styles, diffDistance2d[0]), min), max);
        const scaleY = Math.min(Math.max(getScaleY(event, styles, diffDistance2d[1]), min), max);

        this.pinchStyles.transform.push({ scaleX });
        this.pinchStyles.transform.push({ scaleY });
        this.prevDistance2d = increasedDistance2d;
      }
